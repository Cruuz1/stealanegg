...
local function CreateDropdownOption(Name, Order)
    ...
    Option.MouseButton1Click:Connect(function()
        ...
        if _G.Cruuzz_EggCheckPremium then
            local List = {}
            if SelectedRarities.Secret then table.insert(List, "Secret") end
            if SelectedRarities.Eternal then table.insert(List, "Eternal") end
            if SelectedRarities.Divine then table.insert(List, "Divine") end
            _G.Cruuzz_EggCheckPremium.SetRarities(List)
        end

        -- ✅ Update FarmingManager
        if _G.Cruuzz_FarmingManager then
            local List = {}
            if SelectedRarities.Secret then table.insert(List, "Secret") end
            if SelectedRarities.Eternal then table.insert(List, "Eternal") end
            if SelectedRarities.Divine then table.insert(List, "Divine") end
            _G.Cruuzz_FarmingManager.SetRarities(List)
        end
...
local function ToggleFarm()
    if not _G.Cruuzz_FarmingManager then
        warn("[Cruuzz] FarmingManager not loaded!")
        return
    end
    ...
    if FarmEnabled then
        ...
        _G.Cruuzz_FarmingManager.Enable()
    else
        FarmButton.BackgroundColor3 = Color3.fromRGB(28, 29, 39)
        FarmStroke.Color = Color3.fromRGB(200, 200, 220)
        _G.Cruuzz_FarmingManager.Disable()
    end
end
...
task.spawn(function()
    task.wait(1)
    if _G.Cruuzz_FarmingManager then
        local State = _G.Cruuzz_FarmingManager.IsEnabled()
        FarmEnabled = State
...
task.spawn(function()
    while task.wait(1) do
        if _G.Cruuzz_FarmingManager then
            local CurrentState = _G.Cruuzz_FarmingManager.IsEnabled()
            ...
            if CurrentState ~= UIState then
                ...
                print("[Cruuzz] Farming UI Sync | State: " .. tostring(CurrentState))
            end
        end
    end
end)
...
-- ==================================================
_G.Cruuzz_RefreshFarmingUI = function()
    if _G.Cruuzz_FarmingManager then
        local State = _G.Cruuzz_FarmingManager.IsEnabled()
        FarmEnabled = State
        FarmCheck.Visible = State
        ...
        print("[Cruuzz] Farming Tab UI Refreshed | State: " .. tostring(State))
    end
end
