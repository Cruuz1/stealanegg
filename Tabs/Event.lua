...
ManagerButton.MouseButton1Click:Connect(function()
    if not _G.Cruuzz_ManagerDrone then
        warn("[Cruuzz] ManagerDrone not loaded!")
        return
    end

    local NewState = not _G.Cruuzz_ManagerDrone.IsEnabled()
    UpdateManagerUI(NewState)

    -- ✅ Call Enable/Disable (មិន Save Config)
    if NewState then
        _G.Cruuzz_ManagerDrone.Enable()
    else
        _G.Cruuzz_ManagerDrone.Disable()
    end
end)
...
task.spawn(function()
    task.wait(1)
    if _G.Cruuzz_ManagerDrone then
        local State = _G.Cruuzz_ManagerDrone.IsEnabled()
...
-- ✅ REFRESH FUNCTION
--==================================================
_G.Cruuzz_RefreshEventUI = function()
    if _G.Cruuzz_ManagerDrone then
        local State = _G.Cruuzz_ManagerDrone.IsEnabled()
        UpdateManagerUI(State)
        print("[Cruuzz] Event Tab UI Refreshed | State: " .. tostring(State))
    end
end
...
task.spawn(function()
    while task.wait(1) do
        if _G.Cruuzz_ManagerDrone then
            local CurrentState = _G.Cruuzz_ManagerDrone.IsEnabled()
            local UIState = ManagerCheck.Visible

            if CurrentState ~= UIState then
                UpdateManagerUI(CurrentState)
                print("[Cruuzz] Event UI Sync | State: " .. tostring(CurrentState))
            end
        end
    end
end)
