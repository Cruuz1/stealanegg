...
local function UpdateGetEggBox(Icon, Name, Rate, EggId)
    ...
    GetEggRate.Text = "$" .. (_G.Cruuzz_AutoFarm and _G.Cruuzz_AutoFarm.FormatMoney(Rate or 0) or tostring(Rate or 0)) .. "/s"
    SelectedEggId = EggId
...
local function ToggleGetEgg()
    ...
    if GetEggEnabled then
        ...
        if _G.Cruuzz_AutoFarm then
            _G.Cruuzz_AutoFarm.StartTeleport()
        end
    else
        GetEggCheckButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        GetEggCheckButton.BackgroundTransparency = 0.85
        ...
        -- ✅ Call StopTeleport
        if _G.Cruuzz_AutoFarm then
            _G.Cruuzz_AutoFarm.StopTeleport()
        end
    end
end
...
local function CreateEggEntry(EggData)
    ...
    RateLabel.Text = "$" .. (_G.Cruuzz_AutoFarm and _G.Cruuzz_AutoFarm.FormatMoney(EggData.EarningRate) or tostring(EggData.EarningRate)) .. "/s"
    ...
    SelectButton.MouseButton1Click:Connect(function()
        ...
        if _G.Cruuzz_AutoFarm then
            _G.Cruuzz_AutoFarm.SelectEgg(EggData)
        end
    end)
...
local function RefreshEggList()
    ...
    if not _G.Cruuzz_AutoFarm then return end
    ...
    local Eggs = _G.Cruuzz_AutoFarm.ScanEggs()
