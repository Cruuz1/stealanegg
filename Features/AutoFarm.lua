...
local function EnableAutoFarm()
    ...
    print("[Cruuzz] Auto Farm: ON")
end

local function DisableAutoFarm()
    AutoFarmEnabled = false
    print("[Cruuzz] Auto Farm: OFF")
end
...
local function SelectEgg(EggData)
    SelectedEgg = EggData
    print("[Cruuzz] Selected Egg: " .. EggData.DisplayName .. " ($" .. FormatMoney(EggData.EarningRate) .. "/s)")
end
...
local function StartTeleport()
    if not SelectedEgg then
        warn("[Cruuzz] No Egg Selected")
        return
    end

    local Method = _G.Cruuzz_SelectedMethod or "TeleportFly"
    local Speed = _G.Cruuzz_TeleportSpeed or 300

    print("[Cruuzz] Start Teleport | Method: " .. Method .. " | Speed: " .. tostring(Speed) .. " | Target: " .. SelectedEgg.Id)

    if _G.Cruuzz_TeleportSystem then
        _G.Cruuzz_TeleportSystem.SetMethod(Method)
        _G.Cruuzz_TeleportSystem.SetSpeed(Speed)
        _G.Cruuzz_TeleportSystem.SetTargetId(SelectedEgg.Id)
        _G.Cruuzz_TeleportSystem.Enable()
    end
end
...
local function StopTeleport()
    if _G.Cruuzz_TeleportSystem then
        _G.Cruuzz_TeleportSystem.Disable()
    end
    print("[Cruuzz] Stop Teleport")
end
